...
/**  ###########    general section      ##############     **/
    ...
    const unsigned LOG_BANKS_PER_RANK       = 9;//6;
    ...
    const unsigned LOG_NUM_OF_CHANNELS      = 0; //3;
    ...
    inline Addr addr2PIMaddr(Addr addr) {return (addr & PAGE_MASK);}
    ...
    const uint64_t MATS_PER_PAGE = MAT_PER_SUBARRY*
                                        SUBARRAYS_PER_PIMCTRL*CHIPS_PER_RANK;
...
**/
    ...
    const unsigned     ADDR_BANK_IDX[LOG_BANKS_PER_RANK] = {21,22,23,24,25,26,27,28,29};

    const unsigned     ADDR_RANK_IDX[LOG_RANK_PER_CHANNEL] = {};

    const unsigned     ADDR_CHANNLE_IDX[LOG_NUM_OF_CHANNELS] = {};
...
/**  ###########    operation data section      ##############     **/
    ...
    enum pimOpcode {
        ...
        BARRIER         = 0x1E, // a barrier for scope_relax model
    ...
    // fields belonging to the gem5 communication type (help with
    // assiging pages to the PIM and filling them).
    // This fields are dedicated for the YCSB benchmark we use in this
    // project.
    const unsigned SUBTYPE_BIT_LEN = 4;
    enum pimGEM5CommSubtype {
        ADD_PAGE        = 0,
        REMOVE_PAGE     = 1,
        ADD_FILL_PAGE   = 2,
    };
    const unsigned PAGEIDX_BIT_LEN = 16;
    const unsigned FIELDNUM_BIT_LEN = LOG_MAT_COLS-LOG_BYTE_SIZE; // 9-3 = 6
    const unsigned FIELDLEN_BIT_LEN = LOG_MAT_COLS-LOG_BYTE_SIZE; // 9-3 = 6
    const unsigned RECORDNUM_BIT_LEN = 24; // In units of 1000

    const unsigned SUBTYPE_BIT_START      = OPCODE_BIT_END + 1;
    const unsigned SUBTYPE_BIT_END        =
                            SUBTYPE_BIT_START + SUBTYPE_BIT_LEN - 1;
    const unsigned PAGEIDX_BIT_START      = SUBTYPE_BIT_END + 1;
    const unsigned PAGEIDX_BIT_END        =
                            PAGEIDX_BIT_START + PAGEIDX_BIT_LEN - 1;
    const unsigned FIELDNUM_BIT_START      = PAGEIDX_BIT_END + 1;
    const unsigned FIELDNUM_BIT_END        =
                            FIELDNUM_BIT_START + FIELDNUM_BIT_LEN - 1;
    const unsigned FIELDLEN_BIT_START      = FIELDNUM_BIT_END + 1;
    const unsigned FIELDLEN_BIT_END        =
                            FIELDLEN_BIT_START + FIELDLEN_BIT_LEN - 1;
    const unsigned RECORDNUM_BIT_START      = FIELDLEN_BIT_END + 1;
    const unsigned RECORDNUM_BIT_END        =
                            RECORDNUM_BIT_START + RECORDNUM_BIT_LEN - 1;
    
    const uint64_t SUBTYPE_MASK          =
                    pimGetMask(SUBTYPE_BIT_LEN,SUBTYPE_BIT_START);
    const uint64_t PAGEIDX_MASK          =
                    pimGetMask(PAGEIDX_BIT_LEN,PAGEIDX_BIT_START);
    const uint64_t FIELDNUM_MASK          =
                    pimGetMask(FIELDNUM_BIT_LEN,FIELDNUM_BIT_START);
    const uint64_t FIELDLEN_MASK          =
                    pimGetMask(FIELDLEN_BIT_LEN,FIELDLEN_BIT_START);
    const uint64_t RECORDNUM_MASK          =
                    pimGetMask(RECORDNUM_BIT_LEN,RECORDNUM_BIT_START);
    ...
    inline uint64_t setOpGEM5(uint64_t subtype, uint64_t pageidx,
                            uint64_t k_recordnum,
                            uint64_t fieldlen, uint64_t fieldnum){
        uint64_t op = 0;
        op |= (pimOpcode::GEM5 << OPCODE_BIT_START) & OPCODE_MASK;
        op |= (subtype << SUBTYPE_BIT_START) & SUBTYPE_MASK;
        op |= (pageidx << PAGEIDX_BIT_START) & PAGEIDX_MASK;
        op |= (k_recordnum << RECORDNUM_BIT_START) & RECORDNUM_MASK;
        op |= (fieldlen << FIELDLEN_BIT_START) & FIELDLEN_MASK;
        op |= (fieldnum << FIELDNUM_BIT_START) & FIELDNUM_MASK;
        return op;
    }
